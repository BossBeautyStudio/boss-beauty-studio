...
// Génère 10 accroches Instagram percutantes via Claude.
//
// Flow :
//   1. Authentification     — 401 si absent
//   2. Validation du body   — 400 si champs requis manquants ou vides
...
import { isMockMode, getMockHooks, MOCK_TOKENS_USED } from "@/lib/mock";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { specialite, typeContenu, tonStyle, publicCible } = body;

    if (
      typeof specialite !== "string" ||
      specialite.trim().length === 0 ||
      typeof typeContenu !== "string" ||
      typeContenu.trim().length === 0 ||
      ...
      return NextResponse.json(
        {
          ...
          required: ["specialite", "typeContenu", "tonStyle"],
        },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (err instanceof SubscriptionInactiveError) {
      ...
      console.error("[hooks] assertQuota unexpected error:", err);
      return NextResponse.json(
  ...
  if (isMockMode() || isFreeGeneration) {
    ...
    try {
      const result = await callClaudeJSON<HooksOutput>(buildHooksPrompt(params));
      data = result.data;
      tokensUsed = result.tokens;
    } catch (err) {
      console.error("[hooks] callClaudeJSON error:", err);
      return NextResponse.json(
        {
          error:
  ...
  if (isFreeGeneration) {
    try {
      ...
      console.error("[hooks] incrementFreeQuota error:", err);
    }
  } else if (quotaStatus) {
    try {
      await incrementQuota(userId);
    } catch (err) {
      console.error("[hooks] incrementQuota error:", err);
    }
  }
